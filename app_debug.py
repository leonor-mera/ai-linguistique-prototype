    ...
    if st.button("Analyser ma prononciation maintenant 🎧"):
        ...
        while time.time() - start_time < 10:
            try:
                ...
                st.warning("⚠️ Aucun audio détecté dans ce laps de temps.")
                break

        st.write(f"🔍 [DEBUG] Nombre de blocs audio captés : {len(audio_data)}")

        if audio_data:
            audio_array = np.concatenate(audio_data, axis=1)[0]
            temp_audio_path = "input_debug.wav"
            ...
            reply_audio_path = "reply_debug.mp3"
            tts.save(reply_audio_path)
            st.audio(reply_audio_path, format="audio/mp3")

            os.remove(temp_audio_path)
        else:
            st.warning("⚠️ Aucun audio reçu. Veuillez parler clairement et réessayer.")
